...
4.  [文件处理 (File I/O)](#4-文件处理-file-io)
5.  [数据序列化 (JSON)](#5-数据序列化-json)
6.  [面向对象 (OOP)](#6-面向对象-oop)
7.  [常用内置函数](#7-常用内置函数)
8.  [标准库模块示例](#8-标准库模块示例)
9.  [单元测试断言方法](#9-单元测试断言方法)
...
*   **`list.sort()` (方法)**
    *   **描述**: 对列表进行**永久性**排序（就地修改）。默认升序。可通过 `reverse=True` 参数进行降序排序。
    *   **示例**:
        ```python
